...
public class FMGenerator extends AbstractFMGenerator {
	...
	protected void generateTree(Feature root) {
		...
		while (statistics.getNoFeatures() < numberOfFeatures) {
			...
			// mrh
			//System.out.println("Total #children("+current_feature+"): "+nChildren);
	...
	private void createOr(Feature parent, int nChildren) {
		...
		while (childrenToAdd >= 2) {
			...
			//System.out.println("Creating OR: "+parent+" -> "+childrenOr.toString()); //
			Relation orRelation = addOr(parent, childrenOr);
			
			// mrh: printout for debugging
/*			System.out.print(orRelation.getParent() + " -> "); 							//
			for (Iterator<Feature> f = orRelation.getDestination(); f.hasNext(); ){
	...
	private void createAlternative(Feature parent, int nChildren) {
		...
		while (childrenToAdd >= 2) {
			...
			//System.out.println("Creating ALT: "+parent+" -> "+childrenAlt.toString());		//
			Relation altRelation = addAlternative(parent, childrenAlt);
			
			// mrh: printout for debugging
			/*System.out.print(altRelation.getParent() + " -> "); 						
			for (Iterator<Feature> f = altRelation.getDestination(); f.hasNext(); ){
				System.out.print(f.next()+" ");
			}
			System.out.println();*/
